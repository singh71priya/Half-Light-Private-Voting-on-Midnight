...
export class MidnightWalletProvider implements MidnightProvider, WalletProvider {
  ...
  static async build(logger: Logger, env: EnvironmentConfiguration, seed?: string): Promise<MidnightWalletProvider> {
    ...
    let seeds: WalletSeeds;
    if (!seed) {
      seeds = WalletSeeds.generateRandom();
    } else if (seed.trim().includes(' ')) {
      const cleanMnemonic = seed.trim();
      if (typeof (WalletSeeds as any).fromMnemonic === 'function') {
        seeds = (WalletSeeds as any).fromMnemonic(cleanMnemonic);
      } else {
        try {
          const bip39 = await import('bip39');
          const entropy = bip39.mnemonicToEntropy(cleanMnemonic);
          seeds = WalletSeeds.fromMasterSeed(entropy as any);
        } catch {
          seeds = WalletSeeds.fromMasterSeed(cleanMnemonic as any);
        }
      }
    } else {
      seeds = WalletSeeds.fromMasterSeed(seed.trim() as any);
    }
